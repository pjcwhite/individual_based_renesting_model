;extensions[csv r]; BLOCK OUT IF NOT USING R
...
to setup
  ...
  ;BLOCK OUT IF NOT USING R
  ;r:put "cumdist" observed-nest-initiations-per-day-vector
  ;r:eval "write.csv(cumsum(cumdist),file=\"C:/Users/40007266/Documents/RESEARCH/4 - MANUSCRIPT STAGE PROJECTS/RENESTING - GWCT & UNIVERSITY OF READING/NETLOGO MODEL/temp.csv\",col.names=F,row.names=F)"

  file-open "temp.csv"
  set cumdist csv:from-file "temp.csv"
...
to go
  ...
  reset-first-egg-day
  set renesting-probability first item ticks renesting-probability-vector ; 'item' creates a new list, so 'first' needed to take first element of that new list
  ;if include-observed-nest-initiations? [
  ;set observed-nest-initiations item 2 item ticks observed-nest-initiations-per-day-vector]
  ;set observed-nest-initiations last item ticks observed-nest-initiations-per-day-vector ; TEST TEST TEST
  set cumdist2 first item ticks cumdist
  start-season?
  build
  ;active-day?
  survive-day-eggs?
...
to setup-patches
  ...
  ask patches [set start-day round (random-normal season-start-mean season-start-sd)] ; note that original model started at 16 Mar (day 76) so original season start day is plus 75
                                                                                      ; USE THIS TO GET A MEASURE FOR EACH NEST OF DSP, clucth reduction and clutch size]
end
...
to survive-day-eggs?
   ...
   ask patches with [pcolor = blue] [ifelse random 999 < (egg-dsp * 1000)
...
to hatch?
  ...
  ; need to check this "+ 1" - probability occurs at transition, so need to ensure model is recording correct duration
end
...
to survive-day-chicks?
   ...
   ask patches with [pcolor = green] [ifelse random 999 < (chick-dsp * 1000)
   [set duration-successful duration-successful + 1 set active-day 1]
   [set pcolor yellow set duration-successful 0 set attempts attempts + 1 set active-day 0 set previous-attempt-outcome "F"]]  ; the female converts to post attempt phase and duration-interval is reset to 0
end
...
to fledge?
  ...
  ask patches with [duration-successful >= (chick-duration + egg-duration)] [ifelse random 999 > (chicks-fledged-holder * 1000)
...
to renest?
  ask patches with [pcolor = yellow and (duration-interval = fail-inter-attempt-interval) and (previous-attempt-outcome = "F")] [ifelse random 999 < (renesting-probability * 1000)
  ...
  ask patches with [pcolor = yellow and (duration-interval = success-inter-attempt-interval) and (previous-attempt-outcome = "S")] [ifelse random 999 < (renesting-probability * 1000)
...
105
output nest success
mean [successes] of patches / mean [attempts] of patches
3
1
11
...
61
input nest success
((mean [egg-dsp] of patches) ^ egg-duration)*((mean [chick-dsp] of patches) ^ chick-duration)
3
1
11
...
BLACK REDSTART EMPIRICAL RE-NESTING PROBABABILITY.csv
1
0
String
...
BLACK REDSTART OBSERVED NEST INITIATIONS.csv
1
0
String
...
<b>Observed nest initiations - combined</b> A visualisation of the observed nest initialians entered via the <b>observed-nest-initiations-per-day</b> with Julian day on the x-axis and re-nesting probability on the y-axis. These are all nest inititiations combined, regardless of attempt [.....]
...
- work out what these are and then get some R code up and running to provide basic analyses of these
